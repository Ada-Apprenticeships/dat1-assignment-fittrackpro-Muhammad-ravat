...
ORDER BY registration_count DESC --Order by registration count in descending order to get the member with the most registrations
LIMIT 1;
...
ON m.member_id = ca.member_id
GROUP BY m.member_id
ORDER BY registration_count ASC --Order by registration count in ascending order to get the member with the least registrations
LIMIT 1; 
